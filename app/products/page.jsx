...
'use client'
import { Image, Button, Chip, Spacer } from "@nextui-org/react";
...
function LiImage({ obj }) {
  const ref = useRef()
  return (
    <motion.li key={obj.tooltip} className="h-screen mt-20" ref={ref} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
      <TextStroke baseVelocity={3}>dupri group</TextStroke>
      <TextStroke baseVelocity={-3}>{obj.tooltip}</TextStroke>

      <div className="relative group/image mt-20 ">


        <Image as={NextImage} alt={`${obj.tooltip} image`} src={`/${obj.modalImages[0]}`} width={2000} height={1000} className="shadow z-0 absolute top-0 bottom-0 left-0 right-0 group-hover/image:brightness-50 duration-500 transition-all" />
        ...
        <div className="z-20 absolute top-10 bottom-10 right-10 left-10 opacity-0 group-hover/image:opacity-100 transition-all duration-500">
          ...
          )}>Learn More About {obj.tooltip}</Button>
